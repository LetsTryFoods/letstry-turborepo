...
//             <Text allowFontScaling={false} adjustsFontSizeToFit style={styles.price}>₹{product.price}</Text>
//             {discountPercent > 0 && (
//               <>
//                 <Text allowFontScaling={false} adjustsFontSizeToFit style={styles.mrp}>MRP ₹{product.price + (product.price * discountPercent) / 100}</Text>
...
//             <Text allowFontScaling={false}  style={styles.descriptionText}>{product.description}</Text>
//           </View>
...
//             <Text allowFontScaling={false} adjustsFontSizeToFit style={styles.bottomPrice}>₹{product.price}</Text>
//             {discountPercent > 0 && (
//               <>
//                 <Text allowFontScaling={false} adjustsFontSizeToFit style={styles.bottomMrp}>MRP ₹{product.price + (product.price * discountPercent) / 100}</Text>
...
const ProductDetailsScreen = ({ route, navigation }) => {
  ...
  // --- ✅ YAHAN CHANGES KIYE HAIN (Waapas logScreenView kar diya hai) ---
  useEffect(() => {
    // Yeh check karega ki 'product' null nahi hai
    if (product) {
      
      // --- ✅ ANALYTICS EVENT LOG ---
      // 'logScreenView' hi sahi function hai
      // Is baar yeh crash nahi hoga
      ...
      // Aapka existing similar products ka logic
      if (product.eanCode) {
        fetchSimilarProducts(product.eanCode)
          .then((data) => {
            setSimilarProducts(data)
  ...
  }, [product]) // Yeh 'product' state par depend karta hai
...
if (!product && loading) {
  ...
  // Return null or error state if product is still null after loading
  if (!product) {
    return (
