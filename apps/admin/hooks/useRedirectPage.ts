...
import {
  ...
  useDeleteRedirect,
  Redirect,
} from "@/lib/redirects/useRedirects";
...
export const useRedirectPage = () => {
  ...
  const { mutate: createRedirectMutation, loading: createLoading } =
    useCreateRedirect();
  const { mutate: updateRedirectMutation, loading: updateLoading } =
    useUpdateRedirect();
  const { mutate: deleteRedirectMutation, loading: deleteLoading } =
    useDeleteRedirect();
  ...
  const handleToggleActive = useCallback(
    async (id: string, isActive: boolean) => {
      try {
        ...
        toast.success(
          `Redirect ${isActive ? "activated" : "deactivated"} successfully`,
        );
        await refetch();
      } catch (error: any) {
        toast.error(error.message || "Failed to update redirect");
