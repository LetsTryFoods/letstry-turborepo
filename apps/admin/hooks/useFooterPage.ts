...
import {
  useFooterDetails,
  useCreateFooterDetail,
  useUpdateFooterDetail,
  useDeleteFooterDetail,
} from "@/lib/footer/useFooter";
...
export function useFooterPage() {
  ...
  const [footerToDelete, setFooterToDelete] = useState<{
    id: string;
    companyName: string;
  } | null>(null);
  ...
  const { mutate: createFooter, loading: createLoading } =
    useCreateFooterDetail();
  const { mutate: updateFooter, loading: updateLoading } =
    useUpdateFooterDetail();
  const { mutate: deleteFooter, loading: deleteLoading } =
    useDeleteFooterDetail();
