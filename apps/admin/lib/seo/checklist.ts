...
export function evaluateProductChecklist(p: ProductLike): ChecklistItem[] {
  ...
  const imagesMissingAlt = allVariantImages.filter((i) => !i.alt || i.alt.trim() === '').length;
  items.push({
  ...
  // ---- Long description -----------------------------------------------
  const longDescWordCount = (p.longDescription || '').split(/\s+/).filter(Boolean).length;
  items.push({
    id: 'product-long-description',
    label: 'Long description has 200+ words',
    status: longDescWordCount >= 200 ? 'pass' : longDescWordCount >= 80 ? 'warn' : 'fail',
    hint:
      longDescWordCount === 0
  ...
  items.push({
    ...
    status:
      (p.pros || []).length >= 3 && (p.cons || []).length >= 1 ? 'pass' : 'warn',
    hint: 'Google now emits a "pros & cons" rich result; both arrays should be populated honestly.',
  });

  // ---- Nutrition -------------------------------------------------------
  const hasNutrition = p.nutrition && (p.nutrition.calories || p.nutrition.servingSize);
  items.push({
...
function seoTitleCheck(title?: string | null): ChecklistItem {
  ...
  return { id: 'seo-title', label: `Meta title is set (${len} chars)`, status: 'pass' };
}
...
function seoDescriptionCheck(desc?: string | null): ChecklistItem {
  ...
  return { id: 'seo-description', label: `Meta description is set (${len} chars)`, status: 'pass' };
}
