...
import { GET_ORDER_REPORTS, GET_SALES_BY_STATE } from "@/lib/graphql/reports";
...
export const useShippingInsights = () => {
  ...
  const { data, loading, error, refetch } = useQuery<{
    getShippingInsights: ShippingInsightsData;
...
export const useSalesByState = (
  ...
  const stateData = useMemo(() => {
    if (!data?.getSalesByState) return [] as StateSales[];
    return data.getSalesByState;
  }, [data]);

  return { data: stateData, loading, error, refetch };
};
