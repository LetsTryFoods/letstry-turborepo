...
export function OrderTable({
  ...
  const handlePunchShipment = async (
    ...
    try {
      const result = await punchShipment({
        orderId: order._id,
        serviceType,
        provider,
        pickupLocationName,
    ...
    switch (status) {
      ...
      case "PACKED":
        return (
          ...
          >
            <Package className="h-3 w-3 mr-1" /> Packed
          </Badge>
        );
      case "SHIPPED":
        return (
      ...
      case "IN_TRANSIT":
        return (
          ...
          >
            <Loader2 className="h-3 w-3 mr-1" /> In Transit
          </Badge>
        );
      case "DELIVERED":
        return (
      ...
      case "SHIPMENT_FAILED":
        return (
          <Badge variant="outline" className="border-red-500 text-red-600">
            <XCircle className="h-3 w-3 mr-1" /> Shipment Failed
          </Badge>
        );
      default:
      ...
      <Table>
        <TableHeader>
          <TableRow className="hover:bg-transparent">
            ...
            <TableHead className="text-[11px] font-bold uppercase tracking-wider py-2 px-2.5 h-auto text-right">
              Total
            </TableHead>
            <TableHead className="text-[11px] font-bold uppercase tracking-wider py-2 px-2.5 h-auto">
              Payment
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuLabel className="flex items-center gap-2">
                          ...
                          {getRecommendation(order) === "DTDC" && (
                            <Badge className="bg-orange-100 text-orange-700 hover:bg-orange-100 border-none px-1.5 py-0 text-[10px]">
                              RECOMMENDED
                            </Badge>
                          )}
                        </DropdownMenuLabel>
                        <DropdownMenuItem
                          onClick={() =>
                            handlePunchShipment(order, "B2C SMART EXPRESS")
                          }
                        >
                          B2C Smart Express
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          onClick={() =>
                            handlePunchShipment(order, "B2C PRIORITY")
                          }
                        >
                          B2C Priority
                        </DropdownMenuItem>
                        ...
                        <DropdownMenuItem
                          onClick={() =>
                            handlePunchShipment(order, "Express", "SHIPROCKET")
                          }
                        >
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      {order.orderStatus !== "DELIVERED" && (
                        <>
                          ...
                          {canUpdateTo(order.orderStatus, "PACKED") && (
                            <DropdownMenuItem
                              onClick={() =>
                                onUpdateStatus(order.orderId, "PACKED")
                              }
                            >
                          ...
                          {canUpdateTo(order.orderStatus, "IN_TRANSIT") && (
                            <DropdownMenuItem
                              onClick={() =>
                                onUpdateStatus(order.orderId, "IN_TRANSIT")
                              }
                            >
                      ...
                      <DropdownMenuItem
                        ...
                        disabled={
                          downloadingAll === order._id || downloadingLabel
                        }
                      >
                      ...
                      <DropdownMenuItem
                        onClick={() =>
                          window.open(
                            `${API_BASE_URL}/orders/${order._id}/invoice`,
                            "_blank",
                          )
                        }
                      >
