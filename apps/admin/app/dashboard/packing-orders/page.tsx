...
import {
  GET_ALL_PACKING_ORDERS,
  CLEANUP_ORPHANED_JOBS,
  TRIGGER_REASSIGNMENT_CYCLE,
...
export default function PackingOrdersPage() {
  ...
  const [getEvidence, { data: evidenceData, loading: evidenceLoading }] =
    useLazyQuery(GET_EVIDENCE_BY_ORDER);

  const [cleanupJobs, { loading: cleanupLoading }] = useMutation(
    CLEANUP_ORPHANED_JOBS,
    {
      onCompleted: (data: any) => {
        alert(
          `Cleanup Complete: Removed ${data.cleanupOrphanedJobs.removed} orphaned jobs out of ${data.cleanupOrphanedJobs.checked} checked.`,
        );
      },
      onError: (error) => {
  ...
  const [triggerReassignment, { loading: reassignLoading }] = useMutation(
    TRIGGER_REASSIGNMENT_CYCLE,
    {
      onCompleted: () => {
        alert(
          "Reassignment cycle completed! Unassigned and stale orders have been processed.",
        );
      },
      onError: (error) => {
      ...
      <div className="flex items-center justify-between space-y-2">
        <div>
          ...
          <p className="text-muted-foreground">
            Monitor active packing operations
          </p>
        </div>
        <div className="flex gap-2">
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  <RefreshCw
                    className={`h-4 w-4 ${reassignLoading ? "animate-spin" : ""}`}
                  />
              ...
              <TooltipContent>
                <p>
                  Immediately assigns all unassigned orders to available packers
                </p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent>
                <p>
                  Removes queue jobs for orders that no longer exist in the
                  database
                </p>
              </TooltipContent>
            </Tooltip>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Assigned to packers</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Finished packing</p>
          </CardContent>
        </Card>
      </div>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    ...
                    <div>
                      ...
                      <p className="text-base font-semibold">
                        {selectedOrder.packerName || "Unassigned"}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-muted-foreground mb-1">
                    ...
                    <div>
                      ...
                      <Badge
                        variant={
                          selectedOrder.isExpress ? "default" : "secondary"
                        }
                      >
                    ...
                    <div>
                      ...
                      <Badge
                        variant={
                          selectedOrder.hasErrors ? "destructive" : "secondary"
                        }
                      >
                    ...
                    <>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        <div>
                          ...
                          <p className="text-base font-semibold">
                            {evidence.recommendedBox?.code || "N/A"}
                          </p>
                        </div>
                        <div>
                          <p className="text-sm font-medium text-muted-foreground mb-1">
                        ...
                        <div>
                          ...
                          <Badge
                            variant={
                              evidence.boxMismatch ? "destructive" : "secondary"
                            }
                          >
                      ...
                      <div className="mt-6 space-y-6">
                        <div>
                          <p className="text-sm font-medium text-muted-foreground mb-3">
                            Pre-Pack Images (
                            {evidence.prePackImages?.length || 0})
                          </p>
                          {evidence.prePackImages &&
                          evidence.prePackImages.length > 0 ? (
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                              {evidence.prePackImages.map(
                                (url: string, index: number) => (
                                  <div
                                    ...
                                    onClick={() =>
                                      window.open(getCdnUrl(url), "_blank")
                                    }
                        ...
                        <div>
                          <p className="text-sm font-medium text-muted-foreground mb-3">
                            Post-Pack Images (
                            {evidence.postPackImages?.length || 0})
                          </p>
                          {evidence.postPackImages &&
                          evidence.postPackImages.length > 0 ? (
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                              {evidence.postPackImages.map(
                                (url: string, index: number) => (
                                  <div
                                    key={`post-${index}`}
                                    className="relative aspect-square rounded-md overflow-hidden border cursor-pointer hover:opacity-90 transition-opacity"
