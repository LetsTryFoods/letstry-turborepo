...
export function CouponForm({ onSuccess, onCancel }: CouponFormProps) {
  ...
  const [pendingValues, setPendingValues] = useState<CouponFormValues | null>(
    null,
  );
  ...
  const handleConfirmCreate = async () => {
    ...
    try {
      const input = {
        ...
        endDate:
          pendingValues.hasInfiniteValidity || !pendingValues.endDate
            ? undefined
            : new Date(pendingValues.endDate).toISOString(),
        ...
        >
          ...
          <div className="flex items-center gap-3 p-4 bg-amber-50 border border-amber-200 rounded-lg">
            ...
            <p className="text-sm text-amber-800">
              <strong>Important:</strong> Once a coupon is created, it cannot be
              edited. Please review all details carefully before creating.
            </p>
          </div>
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                  <div className="space-y-0.5">
                    ...
                    <FormDescription>
                      Coupon will be valid indefinitely (controlled only by
                      Active status)
                    </FormDescription>
                  </div>
                  <FormControl>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      End Date {hasInfiniteValidity ? "(Optional)" : "*"}
                    </FormLabel>
                    <FormControl>
                      <Input
                        type="datetime-local"
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                  <div className="space-y-0.5">
                    ...
                    <FormDescription>Enable coupon for use</FormDescription>
                  </div>
                  <FormControl>
                    <Switch
                      checked={field.value}
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                  <div className="space-y-0.5">
                    ...
                    <FormDescription>List coupon in discovery</FormDescription>
                  </div>
                  <FormControl>
                    <Switch
                      checked={field.value}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-3">
              <p>
                Please review the coupon details carefully.{" "}
                <strong>Once created, this coupon cannot be edited.</strong>
              </p>
              {pendingValues && (
                <div className="bg-muted p-3 rounded-lg text-sm space-y-1">
                  <p>
                    <strong>Code:</strong> {pendingValues.code.toUpperCase()}
                  </p>
                  <p>
                  ...
                  <p>
                    ...
                    {pendingValues.discountType === "PERCENTAGE"
                      ? `${pendingValues.discountValue}%`
                      : `₹${pendingValues.discountValue}`}
                  </p>
                  <p>
                    <strong>Status:</strong>{" "}
                    {pendingValues.isActive ? "Active" : "Inactive"}
                  </p>
                  <p>
                    <strong>Visibility:</strong>{" "}
                    {pendingValues.isPublic ? "Public" : "Hidden"}
                  </p>
                </div>
              )}
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={createLoading}>
              Go Back & Edit
            </AlertDialogCancel>
            <AlertDialogAction
