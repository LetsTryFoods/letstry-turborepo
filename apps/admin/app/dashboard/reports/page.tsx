...
// Dynamically import SalesByStateMap to avoid SSR issues (uses Google Maps)
const SalesByStateMap = dynamic(
  () => import("@/components/reports/SalesByStateMap"),
  { ssr: false },
);
    ...
    <div className="space-y-6 mx-6 auto mb-12">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatCompactNumber(summary.totalCustomers)}
            </div>
            <GrowthIndicator
              value={summary.customersGrowth}
              label="vs last period"
            />
          </CardContent>
        </Card>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {shippingData?.avgWeight
                ? `${shippingData.avgWeight.toFixed(2)} kg`
                : "N/A"}
            </div>
            <p className="text-xs text-muted-foreground">
              Across all packed orders
            </p>
          </CardContent>
        </Card>
        {/* 
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {shippingData?.avgDeliveryDays
                ? `${shippingData.avgDeliveryDays} days avg`
                : "N/A"}
            </div>
            <p className="text-xs text-muted-foreground">
              {shippingData?.maxDeliveryDays
                ? `Max ${shippingData.maxDeliveryDays} days`
      ...
      {/* Sales by State - India Map */}
      <div className="mt-8">
        <div className="flex items-center gap-2 mb-4">
          <Globe className="h-5 w-5 text-orange-500" />
          <h2 className="text-xl font-bold tracking-tight">Sales by State</h2>
          <span className="text-sm text-muted-foreground ml-1">
            (India — hover a state for details)
          </span>
        </div>
        <SalesByStateMap
          stateData={stateSalesData}
          loading={stateSalesLoading}
        />
      </div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            ...
            <Select
              value={productSortBy}
              onValueChange={(v) =>
                setProductSortBy(v as "quantity" | "revenue")
              }
            >
      ...
      <div className="grid gap-4 md:grid-cols-2 mt-8">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Orders placed via Website vs Mobile App
            </CardDescription>
          </CardHeader>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">
              Total Track Searches
            </CardTitle>
            <Search className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {trackingData?.getTrackingAnalytics?.totalSearches || 0}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {trackingData?.getTrackingAnalytics?.successfulSearches || 0}
            </div>
            <p className="text-xs text-muted-foreground">
              {trackingData?.getTrackingAnalytics
                ? `${trackingData.getTrackingAnalytics.successRate.toFixed(1)}% success rate`
                : "0% success rate"}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {trackingData?.getTrackingAnalytics?.searchTypeBreakdown.find(
                (s) => s._id === "orderId",
              )?.count || 0}
            </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Latest order tracking search activity
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {trackingData?.getTrackingAnalytics?.recentSearches
              .slice(0, 10)
              .map((search) => (
                <div
                  key={search.id}
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-3 h-3 rounded-full ${search.foundResult ? "bg-green-500" : "bg-red-500"}`}
                    />
                    <div>
                      <p className="font-medium">
                        {search.searchType === "phone"
                          ? "Phone"
                          : search.searchType === "orderId"
                            ? "Order ID"
                            : "AWB"}
                        :
                  ...
                  <div className="text-right">
                    <p
                      className={`text-sm font-medium ${search.foundResult ? "text-green-600" : "text-red-600"}`}
                    >
            ...
            {(!trackingData?.getTrackingAnalytics?.recentSearches ||
              trackingData.getTrackingAnalytics.recentSearches.length ===
                0) && (
              <p className="text-center text-muted-foreground py-8">
                No tracking searches yet
              </p>
            )}
          </div>
        </CardContent>
