...
import {
  BULK_UPSERT_PINCODES,
  CHECK_PINCODE_SERVICEABILITY,
} from "@/lib/pincodes/pincodes.query";
...
function PincodeChecker() {
  ...
  const [checkServiceability, { data, loading }] = useLazyQuery<{
    checkPincodeServiceability: any;
  }>(CHECK_PINCODE_SERVICEABILITY);
  ...
  const isAnyDeliverable =
    result?.smartExpress?.isDeliverable || result?.priority?.isDeliverable;
            ...
            {isAnyDeliverable && (
              <>
                <div>
                  ...
                  {result.smartExpress?.city ||
                    result.priority?.city ||
                    result.city}
                </div>
                <div>
                  <span className="font-semibold text-gray-900">State:</span>{" "}
                  {result.smartExpress?.state ||
                  ...
                  <div className="space-y-2 mt-4 text-sm text-gray-700">
                    <div className="flex justify-between border-b border-emerald-100 pb-1.5">
                      ...
                      <span className="font-bold text-emerald-800">
                        {result.smartExpress.estimatedDays} days
                      </span>
                    </div>
                    <div className="flex justify-between border-b border-emerald-100 pb-1.5">
                      <span className="text-gray-500 font-medium">Zone:</span>
                      <span className="font-bold text-emerald-800">
                        {result.smartExpress.zone || "N/A"}
                      </span>
                    </div>
                  </div>
                )}
                ...
                {result.priority?.isDeliverable && (
                  <div className="space-y-2 mt-4 text-sm text-gray-700">
                    <div className="flex justify-between border-b border-emerald-100 pb-1.5">
                      ...
                      <span className="font-bold text-emerald-800">
                        {result.priority.estimatedDays} days
                      </span>
                    </div>
                    <div className="flex justify-between border-b border-emerald-100 pb-1.5">
                      <span className="text-gray-500 font-medium">Zone:</span>
                      <span className="font-bold text-emerald-800">
                        {result.priority.zone || "N/A"}
                      </span>
                    </div>
                  </div>
                )}
...
export default function PincodesPage() {
  ...
  const [duplicatePincodes, setDuplicatePincodes] = useState<
    { pincode: string; product: string; rows: number[] }[]
  >([]);

  const [bulkUpsert, { loading }] = useMutation<{ bulkUpsertPincodes: number }>(
    BULK_UPSERT_PINCODES,
  );
  ...
  const parseFile = (file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const duplicatesArray = Array.from(duplicatesMap.entries()).map(
          ([key, rows]) => {
        ...
        if (duplicatesArray.length > 0) {
          toast.success(
            `Found ${mappedData.length} unique combinations. ${duplicatesArray.length} entries had duplicates.`,
          );
        } else {
          toast.success(
            `Successfully parsed ${mappedData.length} entries from file.`,
          );
        }
      } catch (err) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      toast.success(
        `Successfully uploaded ${data?.bulkUpsertPincodes || 0} entries.`,
      );
      setFile(null);
      setParsedData([]);
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6">
        Pincode Serviceability Manager
      </h1>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        <div className="mb-4">
          ...
          <p className="text-sm text-gray-600 mb-4">
            Upload an Excel sheet containing the deliverable pincodes. The sheet
            must have the following columns:
            <br />
            <strong>
              PRODUCT, DESTINATION PINCODE, CITY, STATE, ZONE, TAT
            </strong>
          </p>
          <input
            id="pincode-upload"
            ...
            {duplicatePincodes.length > 0 && (
              <div className="mt-8 border border-yellow-200 bg-yellow-50 rounded-lg p-4">
                <h3 className="text-sm font-bold text-yellow-800 mb-2">
                  Duplicate Pincodes Found ({duplicatePincodes.length})
                </h3>
                <p className="text-xs text-yellow-700 mb-4">
                  These pincodes appeared multiple times in your Excel sheet.
                  Only the first occurrence of each was kept for uploading.
                </p>
