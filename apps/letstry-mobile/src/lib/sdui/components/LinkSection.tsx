...
const LinkSection: React.FC<LinkSectionProps> = ({
    ...
    if (link.requiresAuth && !isAuthenticated) {
      Alert.alert(
        ...
        [
          ...
          { text: 'Login Now', onPress: () => ActionEngine.execute({ type: 'NAVIGATE', destination: '/auth/login' }) }
        ]
      );
      return;
    }
  ...
  return (
    <View style={styles.section}>
      {title && <Text style={[styles.sectionTitle, { fontSize: sectionTitleFontSize }]}>{title}</Text>}
      {links.map((link, index) => (
        <TouchableOpacity
          key={link.id}
          style={[
            styles.item,
            index === links.length - 1 && styles.lastItem,
          ]}
        ...
        >
          <View style={styles.itemContent}>
            ...
            />
            <Text style={[styles.itemLabel, { fontSize: labelFontSize }]}>{link.label}</Text>
          </View>
          <Ionicons
