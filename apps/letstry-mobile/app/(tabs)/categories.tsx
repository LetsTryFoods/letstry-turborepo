...
const CategoriesScreen = () => {
  ...
  const [activeCategoryId, setActiveCategoryId] = useState(
    params.categoryId || ALL_PRODUCTS_ID,
  );
  ...
  console.log(
    "[Categories] sduiData components count:",
    sduiData?.components?.length,
  );
  ...
  const { data: categoriesData } = useQuery(GET_ROOT_CATEGORIES, {
  ...
  const activeCategoryName = isAll
    ? "All Products"
    : (categories as any[]).find((c) => c.id === activeCategoryId)?.name ||
      "Categories";
  ...
  const stickyComponents = sduiComponents.filter(
    (c: any) => c.props?.isSticky !== false,
  );
  const scrollableComponents = sduiComponents.filter(
    (c: any) => c.props?.isSticky === false,
  );
          ...
          <View
            ...
            style={[
              styles.header,
              {
                ...
                borderBottomColor: headerStyle.borderBottomColor || "#F0F0F0",
                borderBottomWidth: headerStyle.borderBottomWidth ?? 1,
              },
            ]}
          >
            <View style={styles.headerContent}>
              ...
              >
                <Ionicons
                  name="chevron-back"
                  size={24}
                  color={headerStyle.iconColor || "#333"}
                />
              </TouchableOpacity>
              <Text
                style={[
                  styles.headerTitle,
                  { color: headerStyle.textColor || "#333" },
                ]}
              >
                {activeCategoryName}
              </Text>
          ...
          <View key={index} style={styles.mainContent}>
            ...
            <View
              style={[
                ...
                {
                  backgroundColor:
                    item.props?.styleConfig?.gridBackgroundColor || "#FFFFFF",
                },
              ]}
            >
              <CategoryProductGrid
                ...
                ListHeaderComponent={
                  scrollableComponents.length > 0 ? (
                    <View>
                      {scrollableComponents.map((comp: any, idx: number) =>
                        renderComponent(comp, idx, true),
                      )}
                    </View>
                  ) : null
                }
  ...
  return (
    <View style={styles.container}>
      ...
      {stickyComponents.map((item: any, index: number) =>
