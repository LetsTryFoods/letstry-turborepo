...
export class ProductValidationService {
    ...
    try {
      ...
      try {
        ...
        this.logger.log(
          ...
          { variantId: productId, productId: product._id },
          'CartModule',
        );
        return product;
      } catch (variantError) {
        this.logger.error(
          'Product not found (tried both product ID and variant ID)',
          { productId },
          'CartModule',
        );
  ...
  async validateProductAvailability(productId: string) {
    ...
    const variantId = await this.validateVariantAvailability(
      product,
      productId,
    );
  ...
  async isVariantId(productId: string, product: any): Promise<boolean> {
    return product.variants.some(
      (variant: any) => variant._id.toString() === productId,
    );
  }

  async validateSpecificVariant(
    product: any,
    variantId: string,
  ): Promise<void> {
    ...
    if (variant.availabilityStatus !== 'in_stock') {
      ...
      throw new BadRequestException(
        'Product variant is currently out of stock',
      );
    }
  }
  ...
  validateProductHasAvailableVariants(product: any, productId: string): void {
    const hasAvailableVariant = product.variants.some(
      (v: any) => v.availabilityStatus === 'in_stock' && v.isActive,
    );
    if (!hasAvailableVariant) {
      this.logger.error(
        'No available variants for product',
        { productId },
        'CartModule',
      );
  ...
  getDefaultVariant(product: any): any {
    const defaultVariant = product.variants.find(
      (v: any) =>
        v.isDefault && v.isActive && v.availabilityStatus === 'in_stock',
    );
    if (defaultVariant) {
      return defaultVariant;
    }
    const firstAvailableVariant = product.variants.find(
      (v: any) => v.isActive && v.availabilityStatus === 'in_stock',
    );
