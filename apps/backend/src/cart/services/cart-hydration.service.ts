...
export class CartHydrationService {
  ...
  async hydrate(cart: CartDocument): Promise<CartDocument> {
    ...
    if (cart.shippingAddressId) {
      try {
        shippingAddress = await this.addressModel
          .findById(cart.shippingAddressId)
          .exec();
      } catch (error) {
        this.logger.warn(
    ...
    await this.cartCalculationService.recalculateCart(
      cart,
      shippingAddress ? { pincode: shippingAddress.postalCode } : undefined,
    );
