...
export class TrackingCronService implements OnModuleInit {
  ...
  async onModuleInit() {
    const pollIntervalHours =
      this.configService.get<number>('dtdc.tracking.pollIntervalHours') || 1;
  ...
  async triggerTrackingSync(): Promise<void> {
    ...
    this.trackingLogger.logInfo('Manual tracking sync triggered via API', {
