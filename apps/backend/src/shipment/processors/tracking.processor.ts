...
export class TrackingProcessor extends WorkerHost {
  ...
  async process(job: Job<any>): Promise<void> {
    if (job.name === 'sync-all-tracking') {
      ...
      try {
        ...
        this.shipmentLogger.logShiprocketWebhookResult(
          awb,
          true,
          internalStatus,
        );
  ...
  private async handleSyncAllTracking(): Promise<void> {
    ...
    const activeShipments =
      await this.shipmentService.findActiveShipmentsForTracking('DTDC');
    ...
    for (const shipment of activeShipments) {
      ...
      this.trackingLogger.logJobEnqueued(
    ...
    const srShipments =
      await this.shipmentService.findActiveShipmentsForTracking('SHIPROCKET');
    ...
    for (let i = 0; i < srAwbs.length; i += 50) {
      ...
      try {
        ...
        this.logger.error(
          'Shiprocket bulk tracking batch failed: ' + err.message,
        );
      }
    }
  }

  private async handleSyncShipment(
    awbNumber: string,
    shipmentId: string,
  ): Promise<void> {
    try {
      ...
      if (newEvents && newEvents.length > 0) {
        ...
        this.trackingLogger.logShipmentSyncSuccess(
          awbNumber,
          shipmentId,
          newEvents.length,
        );
      ...
      this.trackingLogger.logError(
        `Failed to sync tracking for shipment ${awbNumber}`,
        error,
      );
      throw error;
    }
