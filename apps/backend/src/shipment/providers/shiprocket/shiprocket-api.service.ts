...
export class ShiprocketApiService {
  ...
  private async request(method: string, endpoint: string, data?: any, isRetry = false): Promise<any> {
    ...
    try {
      ...
      this.shipmentLogger.logApiCall(logMessage, method, response.status, duration, 'SHIPROCKET');
      ...
      if (error.response?.status === 401 && !isRetry) {
        this.logger.warn('Shiprocket API 401 Unauthorized. Clearing token and retrying...');
      ...
      this.shipmentLogger.logApiError(logMessage, method, errMessage, duration, 'SHIPROCKET');
      this.logger.error(`Shiprocket API error on ${method} ${endpoint}: ${errMessage}`, error.response?.data);
      
      throw new HttpException(errMessage, error.response?.status || HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }

  async createForwardShipment(payload: any): Promise<any> {
    this.logger.log(`Creating forward shipment for order ${payload.order_id}`);
    this.shipmentLogger.logInfo(`Creating forward shipment for order ${payload.order_id}`, undefined, 'SHIPROCKET');
  ...
  async trackByAwb(awbNumber: string): Promise<any> {
    ...
    this.shipmentLogger.logInfo(`Tracking AWB: ${awbNumber}`, undefined, 'SHIPROCKET');
  ...
  async trackBulkAwbs(awbs: string[]): Promise<Record<string, any>> {
    ...
    this.shipmentLogger.logInfo(`Bulk tracking AWBs: ${awbs.length} passed.`, undefined, 'SHIPROCKET');
  ...
  async cancelOrder(srOrderId: string): Promise<boolean> {
    ...
    this.shipmentLogger.logInfo(`Cancelling order: ${srOrderId}`, undefined, 'SHIPROCKET');
  ...
  async getPickupLocations(): Promise<any[]> {
    ...
    this.shipmentLogger.logInfo(`Fetched pickup locations from Shiprocket`, undefined, 'SHIPROCKET');
