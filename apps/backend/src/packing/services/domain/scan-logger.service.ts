...
export class ScanLoggerService {
  ...
  logScanRequest(
    mutation: string,
    payload: { packingOrderId: string; ean: string; packerId: string },
  ) {
  ...
  logScanValidation(
    ...
    ean: string,
    validation: { isValid: boolean; errorType?: string; item?: any },
  ) {
