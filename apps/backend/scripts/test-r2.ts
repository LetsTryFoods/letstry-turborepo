...
async function bootstrap() {
  ...
  try {
    const testData = await uploadService.generatePresignedUploadData(
      'test-image.png',
      'image/png',
    );
    ...
    if (
      testData.uploadUrl.includes('r2.cloudflarestorage.com') &&
      testData.uploadUrl.includes('cdn-bucket-main')
    ) {
      console.log(
