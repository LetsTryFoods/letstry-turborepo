...
// Paths
const SWIGGY_CSV_PATH = path.resolve(
  '/Users/apple/Desktop/Swiggy Master File - New File.csv',
);
const OLDER_CSV_PATH = path.resolve(
  "/Users/apple/Desktop/Let's Try - Products Details - Sheet1.csv",
);
...
async function main() {
  ...
  const compareBy =
    args.find((arg) => arg.startsWith('--by='))?.split('=')[1] || 'all';

  if (!['sku', 'old-ean', 'new-ean', 'all'].includes(compareBy)) {
    console.error(
      'Invalid comparison method. Use --by=sku, --by=old-ean, --by=new-ean, or --by=all',
    );
    process.exit(1);
  }

  console.log(
    `🚀 Starting Final Three-File Comparison (By: ${compareBy})...\n`,
  );
  ...
  try {
    ...
    console.log(
      `✅ Loaded ${swiggyProducts.length} filtered products from Swiggy file (By: ${compareBy}).`,
    );
    ...
    await new Promise((resolve, reject) => {
      if (!fs.existsSync(OLDER_CSV_PATH)) {
        return reject(
          new Error(`Older CSV file not found at ${OLDER_CSV_PATH}`),
        );
      }
    ...
    for (const prod of swiggyProducts) {
      ...
      let inOlder = false;
      if (compareBy === 'sku' || compareBy === 'all') {
        if (prod.sku && prod.sku !== 'N/A' && olderEans.has(prod.sku))
          inOlder = true;
      }
      if (compareBy === 'new-ean' || compareBy === 'all') {
        if (prod.newEan && prod.newEan !== 'N/A' && olderEans.has(prod.newEan))
          inOlder = true;
      }
      if (compareBy === 'old-ean' || compareBy === 'all') {
        if (prod.oldEan && prod.oldEan !== 'N/A' && olderEans.has(prod.oldEan))
          inOlder = true;
      }
