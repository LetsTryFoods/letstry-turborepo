...
const DETAILS_CSV_PATH = path.resolve(
...
async function main() {
  console.log(
    '--- List Comparison: Missing Products vs Detailed Master Sheet ---',
  );

  if (!fs.existsSync(MISSING_JSON_PATH)) {
    console.error(
      'Error: missing-products.json not found. Run the database comparison script first.',
    );
    process.exit(1);
  }

  // 1. Load Missing Products (JSON)
  const missingProducts: MissingProduct[] = JSON.parse(
    fs.readFileSync(MISSING_JSON_PATH, 'utf-8'),
  );
  ...
  console.log(
    `Loaded ${detailsProducts.length} valid products from Details CSV.`,
  );
  ...
  console.log(
    `2. NOT found in Details list: ${missingNotFoundInDetails.length}`,
  );
