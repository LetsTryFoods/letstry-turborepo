...
async function backup() {
  ...
  try {
    ...
    try {
      execSync(`mongodump --uri="${uri}" --db=${dbName} --out="${backupDir}"`, {
        stdio: 'inherit',
      });
      console.log(
        `Backup completed successfully using mongodump. Files saved in: ${backupDir}`,
      );
    } catch (dumpError) {
      console.warn(
        'mongodump failed or is not installed. Falling back to manual collection export...',
      );
      ...
      console.log(
        `Manual backup completed. ${collections.length} collections exported as JSON to: ${backupDir}`,
      );
    }
  } catch (error) {
