...
const DATABASE_URL =
  ...
  'mongodb://admin:password@localhost:27017/letstry_dev?authSource=admin';
...
const GENERIC_ALLERGENS =
  'May contain traces of nuts, wheat, and dairy products';
...
async function downloadImagesFromDrive(
  driveLink: string,
  skuCode: string,
  imagesFolder: string,
): Promise<boolean> {
  ...
  try {
    ...
    try {
      ...
      try {
        const output = execSync(rcloneCommand, {
          encoding: 'utf-8',
          stdio: 'pipe',
        });
        logger.info(output);
      ...
      try {
        ...
        if (foundPath && fs.existsSync(foundPath)) {
          ...
          if (imageFiles.length > 0) {
            ...
            logger.success(
              `Copied ${imageFiles.length} images from local Drive`,
            );
            return true;
          }
        }
...
async function uploadImageToS3(
  ...
  try {
    ...
    const webpBuffer = await sharp(imagePath).webp({ quality: 75 }).toBuffer();
    ...
    try {
      presignedResponse = await axios.post(
      ...
      logger.error(
        `    → Response Data: ${JSON.stringify(presignedError.response?.data)}`,
      );
...
async function processImages(
  ...
  if (
    !fs.existsSync(productImagesPath) ||
    fs.readdirSync(productImagesPath).length === 0
  ) {
    logger.info(`Attempting to download images for SKU ${skuCode}...`);
    const downloaded = await downloadImagesFromDrive(
      driveLink,
      skuCode,
      imagesFolder,
    );
    ...
    if (!downloaded) {
      logger.warn(
        `Image folder not found or empty for SKU ${skuCode}: ${productImagesPath}`,
      );
...
function processRow(row: CSVRow): ProcessedProduct {
  ...
  const productNameWithoutWeight = skuName
    .replace(/\d+\s*(g|gm|gms|kg|ml|l|Gm|Gms)$/i, '')
    .trim();
...
async function importProducts(
  ...
  logger.info(
    'This script will attempt to download images automatically using:',
  );
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      logger.info(
        `    - Price: ₹${productData.variants[0].price} (${DISCOUNT_PERCENT}% off)`,
      );
      ...
      if (dryRun) {
        ...
        if (existingProduct) {
          logger.warn(
            `  ⚠️  Product already exists (slug: ${productData.slug} or SKU: ${productData.variants[0].sku})`,
          );
          stats.skipped++;
          skippedProducts.push({
            row: rowIndex,
...
async function main() {
  ...
  const csvPath = path.join(
    __dirname,
    '../../../file_a_swiggy_not_in_db - Missing Products.csv',
  );
  ...
  if (!dryRun && !fs.existsSync(imagesFolder)) {
    ...
    console.warn(
      `Creating folder. Please organize images as: product-images/{SKU_CODE}/image1.jpg`,
    );
