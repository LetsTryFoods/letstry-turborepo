...
function replaceUrl(url: string): { newUrl: string; changed: boolean } {
  ...
  if (url.startsWith('https://') && !url.includes('cloudfront.net'))
...
async function migrate() {
  ...
  while (await categoryCursor.hasNext()) {
    ...
    if (i.changed) {
      updateData.imageUrl = i.newUrl;
      changed = true;
    }
    if (doc.seo?.ogImage) {
      const s = replaceUrl(doc.seo.ogImage);
      if (s.changed) {
        updateData['seo.ogImage'] = s.newUrl;
        changed = true;
      }
    }
    if (changed) {
