...
const CameraScreen = ({ navigation, route }) => {
  ...
  const handleBarcodeScan = async ({ data }) => {
    ...
    try {
      ...
      const result = response?.data?.batchScanItems;
      ...
      if (!validation?.isValid) {
        Vibration.vibrate();
        Alert.alert("Invalid Item", validation?.errorType || "Unknown error");
        return;
      }

      setItems(prev => {
        const updated = [...prev];
        const index = updated.findIndex(i => i.ean === validation.ean);
        if (index !== -1) updated[index].scannedQty += 1;
        return updated;
      });
