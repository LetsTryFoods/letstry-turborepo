  ...
  static async bookShipment(order: LegacyOrder): Promise<PunchResult> {
    try {
      ...
      const payload = {
        ...
        order_date: new Date(typeof order.createdAt === 'string' ? order.createdAt : order.createdAt.$date).toISOString().split('T')[0],
        pickup_location: order.pickupLocation || process.env.SHIPROCKET_DEFAULT_PICKUP_LOCATION || 'Primary',
      ...
      const response = await axios.post(`${SHIPROCKET_API_URL}/shipments/create/forward-shipment`, payload, {
...
export class DtdcClient {
  ...
  static async bookShipment(order: LegacyOrder): Promise<PunchResult> {
    try {
      ...
      const response = await axios.post(`${baseUrl}consignment/softdata`, payload, {
        headers: { 'api-key': apiKey }
      });

      if (response.data?.status !== 'OK' || !response.data?.data?.[0]?.success) {
        throw new Error(response.data?.data?.[0]?.message || 'DTDC Booking failed');
      }
      ...
      let labelUrl = '';
      try {
        const labelResp = await axios.get(`${baseUrl}consignment/shippinglabel/stream?reference_number=${awbNumber}&label_code=SHIP_LABEL_4X6&label_format=pdf`, {
          headers: { 'api-key': apiKey },
