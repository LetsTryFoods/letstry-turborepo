...
interface RichContentProps {
  product: {
    ...
    certifications?: { name: string; number?: string | null; iconUrl?: string | null }[] | null;
    lifestyleImages?: { url: string; alt: string; caption?: string | null }[] | null;
...
export function ProductRichContent({ product }: RichContentProps) {
  return (
    <div className="space-y-10 max-w-4xl">
      ...
      <ProductWhoFor audience={product.audience} occasions={product.occasions} />
...
function ProductHealthBenefits({ html }: { html?: string | null }) {
  if (!html) return null;
  return (
    <section aria-labelledby="benefits-heading" className="bg-green-50 rounded-2xl p-6 sm:p-8 border border-green-100">
  ...
  return (
    <section aria-labelledby="who-for-heading">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {hasAudience && (
          <div>
            <h3 className="text-sm uppercase tracking-wide text-gray-500 mb-2">Best for</h3>
            <ul className="flex flex-wrap gap-2">
              {audience!.map((a) => (
                <li key={a} className="px-3 py-1 rounded-full bg-amber-50 text-amber-900 text-sm border border-amber-200">
                  {a}
                </li>
              ))}
            </ul>
        ...
        {hasOccasions && (
          <div>
            <h3 className="text-sm uppercase tracking-wide text-gray-500 mb-2">Great for</h3>
            <ul className="flex flex-wrap gap-2">
              {occasions!.map((o) => (
                <li key={o} className="px-3 py-1 rounded-full bg-rose-50 text-rose-900 text-sm border border-rose-200">
                  {o}
                </li>
              ))}
            </ul>
      ...
      <div className="grid sm:grid-cols-2 gap-6">
        {hasPros && (
          <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-5">
            <h3 className="text-sm font-semibold text-emerald-900 mb-3">What we love</h3>
        ...
        {hasCons && (
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-5">
            <h3 className="text-sm font-semibold text-amber-900 mb-3">Things to know</h3>
            <ul className="space-y-2">
              {cons!.map((c, i) => (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {images.map((img, i) => (
          <figure key={i} className="rounded-xl overflow-hidden bg-gray-100">
            ...
            {img.caption && (
              <figcaption className="p-2 text-xs text-gray-600">{img.caption}</figcaption>
            )}
          </figure>
        ))}
      </div>
...
function ProductVideo({
  ...
  return (
    <section aria-labelledby="video-heading">
      <SectionHeading id="video-heading">{title || 'See it in action'}</SectionHeading>
      {ytMatch ? (
        ...
        <a href={url} target="_blank" rel="noopener noreferrer" className="text-[#0C5273] underline">
          Watch the video →
        </a>
      )}
...
function ProductCertifications({
  ...
  return (
    <section aria-labelledby="certs-heading">
      <SectionHeading id="certs-heading">Certifications & standards</SectionHeading>
        ...
        {certifications.map((c, i) => (
          ...
          >
            ...
            <span className="text-sm">
              ...
              {c.number ? <span className="text-gray-500"> · {c.number}</span> : null}
            </span>
          </li>
        ))}
      </ul>
...
function ProductFssai({ license }: { license?: string | null }) {
  ...
  return (
    <section className="text-sm text-gray-500 border-t border-gray-100 pt-4">
      FSSAI licence: <span className="font-medium text-gray-700">{license}</span>
    </section>
  );
}
...
function ProductNutritionTable({
  ...
  return (
    <section aria-labelledby="nutrition-heading">
      <SectionHeading id="nutrition-heading">Nutrition information</SectionHeading>
      <div className="overflow-x-auto">
        <table className="min-w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
          <tbody>
            {rows.map((r) => (
              <tr key={r.label} className="border-b border-gray-100 last:border-b-0">
                <th scope="row" className="text-left p-3 bg-gray-50 font-medium text-gray-700 w-1/2">
                  {r.label}
                </th>
      ...
      <p className="text-xs text-gray-500 mt-2">
        Values are typical and may vary slightly batch to batch. Always
        refer to the pack for the authoritative label.
      </p>
    </section>
  );
}
