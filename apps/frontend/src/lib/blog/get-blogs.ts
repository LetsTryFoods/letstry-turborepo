...
export async function getActiveBlogs(): Promise<Blog[]> {
  ...
  const queryString =
    queryObj?.loc?.source?.body || queryObj?.value || queryObj?.toString();

  try {
    const data = await client.request<GetActiveBlogsResponse>(queryString);
...
export async function getBlogBySlug(slug: string): Promise<Blog | null> {
  ...
  try {
    const data = await client.request<GetBlogBySlugResponse>(queryString, {
      slug,
    });
...
export async function getAllBlogs(): Promise<Blog[]> {
  ...
  try {
    const data = await client.request<GetAllBlogsResponse>(queryString);
    return data.blogs;
  } catch (error) {
