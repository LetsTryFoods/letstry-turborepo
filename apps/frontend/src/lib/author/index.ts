...
export async function getAuthorBySlug(slug: string): Promise<Author | null> {
  ...
  try {
    const data = await client.request<{ authorBySlug: Author }>(
      GET_AUTHOR_BY_SLUG,
      { slug },
    );
...
export async function getTeamMembers(): Promise<Author[]> {
  const client = createServerGraphQLClient();
  try {
    const data = await client.request<{ teamMembers: Author[] }>(
      GET_TEAM_MEMBERS,
    );
...
export async function getActiveAuthors(): Promise<Author[]> {
  const client = createServerGraphQLClient();
  try {
    const data = await client.request<{ activeAuthors: Author[] }>(
      GET_ACTIVE_AUTHORS,
    );
